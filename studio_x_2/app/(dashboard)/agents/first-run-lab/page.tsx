...
// A1 first-run-lab — THROWAWAY variant harness (?v=1|2|3, ?s=<phase id>).
// Judged by the 3-lens panel, winner folded into the real wizard landing,
// then this route + components/first-run-lab/* are DELETED.
...
import { SCENARIOS } from "@/components/first-run-lab/spec"
import { Variant1 } from "@/components/first-run-lab/variant-1"
import { Variant2 } from "@/components/first-run-lab/variant-2"
import { Variant3 } from "@/components/first-run-lab/variant-3"

const VARIANTS = {
  "1": { label: "1 · Ceremony", Component: Variant1 },
  "2": { label: "2 · Instant-feel", Component: Variant2 },
  "3": { label: "3 · Working session", Component: Variant3 },
} as const

type VariantKey = keyof typeof VARIANTS

function FirstRunLab() {
  ...
  function setParam(key: "v" | "s", value: string) {
    ...
    next.set(key, value)
    router.replace(`/agents/first-run-lab?${next.toString()}`)
  }
  ...
  return (
    <>
      <PageHeader
        title="First-run lab"
        description="A1 variant harness — judge round only, not a product surface."
      />
        ...
        <div className="mx-auto max-w-5xl space-y-4">
          ...
          <div className="rounded-lg border bg-muted/20 px-4 py-3">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
              Must communicate in this phase
            </p>
...
export default function FirstRunLabPage() {
  return (
    <Suspense>
      <FirstRunLab />
    </Suspense>
