// figma-layers-runner — runs inside use_figma with `const JOB = {...}` prepended
// (compose with scripts/compose-figma-layers.mjs). Rebuilds a captured page
// (scripts/dom-layers.mjs output) as NATIVE editable nodes — rectangles and
// text — inside a frame, next to a locked screenshot frame, under one section
// on page Sandbox New. Chunked: JOB.step = "init" creates the section + the two
// frames and returns their ids; JOB.step = "layers" appends JOB.layers to the
// editable frame JOB.frameId. Every text layer is a real text node.
...
if (JOB.step === "init") {
  ...
  // Left: the locked screenshot (reference). Right: the editable rebuild.
  ...
  // Rationale under the editable frame (hero recipe: a white card with the why).
  let extra = 0
