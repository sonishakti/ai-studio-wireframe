...
| 2026-07-09 | **Depth collapse: the engine comes back inline (studio_x_2, user-directed).** REVERSES the 2026-07-07 lock that moved the model stack out of builder Step 1 into `/agents/playground`. Owner: *"Multi modal vs stack selection is nested in too deep. + voice edition flow also feels nested too deep in. We want everything accessible within this page."* Shipped: (1) `StackConfig` inline in Step 1, editing `draft.stack`; (2) the **pipeline toggle (STT·LLM·TTS vs Multimodal LLM) lifted out of the "Customize models" disclosure** into a first-class visible choice — only vendor dropdowns still nest; (3) a new **`VoiceEditorSheet`** creates/forks/edits voices on the builder page (presets stay immutable → editing forks a copy; the sheet snapshots `draft.stack` instead of duplicating an engine editor); (4) import selects the imported voice in place instead of bouncing to the Playground; (5) Step 1 renamed "Voice & models". **Principle: depth should track how rare/expert a decision is, not how it was historically filed.** The Playground route survives for ⌘K + the agents list. |
...
| 2026-07-09 | **Web Widget studio ships — the missing Deploy › Web Widget page (Figma 847-17167, `737f1bf`).** The `/deploy/web-widget` redirect stub (wizard-only config) becomes the full configurator from the canonical frame "04_Deploy_Web_Widget_(From_App_Builder)": two columns — config sections (Behaviour · Appearance · Text · Branding expanded; Semantic Colors · Input Fields · UI Elements collapsed, per the frame) and a sticky Live Preview (Collapsed · Voice Mode · Chat Mode) — plus Select Agent + Get Code/Embed (snippet copy → toast). **Preview colors are USER CONFIG, so inline styles are correct there; the chrome stays on tokens.** Reuses `AgentSphere` as the voice blob. Wizard web step keeps the quick path + links out to the studio; the channels panel's existing `/deploy/web-widget` href now lands on a real page. Figma defects flagged, NOT adopted: "An error occured" placeholder (we ship "occurred"); the frame's pre-revamp sidebar (content canonical, sidebar not). NOT future-scope gated — baseline page, not roadmap. |
...
| 2026-07-09 | **D1 built — batch throttling detail view "paced ≠ failed" (P0 spine 5/6; HELD for local review).** Research: platforms show a flat "Sent" counter or spinner, so every unexplained stall reads as broken; Twilio's own guidance is queue-and-retry-not-drop, but NO product tells the operator "you're paced, and that's fine." The whitespace = saying it. Replaced the `/deploy/batch-calls/[id]` redirect with a real page. Judge round (3 variants): V2 "reassurance-first" won the load-bearing is-this-working lens (8.8) + fold-fit (8.4) — one leading StateBanner states the verdict, flipped primary↔destructive; V1 "ops console" won operator-answers (8.8, complete telemetry) and was grafted in; V3 "timeline" disqualified on the core lens (red disposition nodes rendered inside the PACED flow → calm case reads broken). **Durable rules:** (1) **paced = primary/working, NEVER warning tone**; degraded (circuit-breaker) = destructive — the amber/red split resolved by making the auto-pause a true `degraded` pacing state, not `paused`; (2) `deploymentHealth()` maps degraded→unhealthy so the Monitor HealthDot and the detail banner agree, paced stays healthy; (3) every zero-progress moment shows `batchRuntime.reason` verbatim; (4) full disposition taxonomy (busy/no-answer retry vs disconnected/wrong-number flagged-and-skipped), bars scaled to total; (5) `batchEta()` = pace × remaining queue (no competitor exposes this); (6) composes with X1 (cap headroom) + A6 (AddLinesSheet at the wall) — the degraded banner is the single **D6-alerting** hook. Data: `Deployment.batchRuntime`, `PACING_META`, `DISPOSITION_META`. Commit `f07b862`; doc `references/design/d1-batch-throttling.html`. NOT deployed. |
| 2026-07-09 | **F-Eval built — evals/simulation + the Talk-surface orb fix (P0 spine 6/6, LAST; HELD for local review).** Closes the 5/5 table-stakes gap (all competitors shipped evals, Agora had none) AND the #1 recurring user-test trust break across ALL THREE sessions (the "Talk to it" test = a pulsing orb, no transcript/state/"simulated" label). Same root cause, one fix: a test proves it worked by SHOWING the conversation. Research → eval object model `Suite → Case{persona, assertions[]} → Run → verdict + transcript + failed assertion`. Judge round (3 variants): V1 "suite table" won author-to-value (8.0) + fold-fit (8.4) + was the ONLY true orb-removal; V2 "conversation-first" won closes-the-gap (9.0) but couldn't scale; shipped = V1 table + V2's inline verdict-coupling (failing assertion anchored onto the offending transcript turn) on a shared `SimTranscript` primitive. **Durable rules:** (1) every test surface shows a live transcript + explicit listening/thinking/speaking state + a loud "Simulated" banner — never a bare orb, never let a sim look like a real call; (2) rubric assertions are plain-language "PASS if…"; (3) "save a real call as a test" is whitespace (pre-fill persona+transcript, ask only the assertion). Folded: `TestsSection` → builder Test step; `SimTranscript` → the identity-card Talk panel (the real-surface fix). Data: `EvalSuite/EvalCase/EvalRun` + `EVAL_SUITE`/`EVAL_RUN`. Commit `0e4c3c2`; doc `references/design/f-eval-simulation.html`. **P0 SPINE COMPLETE — all 6 (X1·A1·A6·A3·D1·F-Eval) built; A3/D1/F-Eval held for local review before deploy.** |
| 2026-07-28 | **v4 builder — hot path collapses to Voice · Channel · Context + Go Live deploy panel (owner-directed, commit `6ce0c4d`).** REVERSES the 06-11 one-agent-one-channel lock: **channels are MULTI-SELECT** (`AgentDraft.channels: DeployChannel[]` — Inbound · Batch · Web widget · WhatsApp-soon-inert · Code/SDK) and **inbound links MULTIPLE numbers** (`numberIds[]`). (1) **Voice = two handles only** — tier cards "Agora Cheapest/Balanced/Best" (the cheapest/balanced/fastest presets; the tier's TTS vendor filters the voice quick-picks, tier-switch remaps the voice same-gender with a toast) + inline sound test; architecture/manual vendors/history/speech tuning → one Advanced slide-out. Voice picks adopt the voice's SOUND, not its engine (the tier is the cost handle; a voice must not silently reset it — imports still carry their engine). (2) **Go Live = the deploy panel**: batch becomes **CAMPAIGNS** (`campaigns[]`, several per agent — own CSV in a half-and-half editor grid, caller ID [single; load-balancing deferred], language/region tag, window/concurrency/retries, launch; duplicate + **re-run**; parallel runs = multiple running rows) + the inbound call settings relocated from Channel + StepAnalysis reframed **"Structured outputs"** + review/pre-flight (one check row + manifest line per campaign). Hang-up/pacing/transfer stay AGENT-level in a "Batch call behavior" sheet. (3) **Template picker moved out of the prompt to a header chip** next to the name (confirm before overwriting a non-empty prompt). (4) **Testing leaves the column**: header Test button toggles a DOCKED drag-resizable right panel (Talk · Scenarios · Widget; width persisted `sx:test_panel_w`; Sheet below lg) — agent-preview-panel + Talk sheet deleted. (5) **Create dialog is AI-first**: two-line describe box → mock keyword inference picks template+channels (said out loud in the landing toast), templates below as a one-per-line radio list. `migrateDraft` upgrades stored drafts; legacy `?step=1-7` + palette events map via `LEGACY_STEP_MAP` (6 → Test panel). Deselecting a channel keeps its config (re-select restores); deselecting a LIVE channel warns with Undo. |
| 2026-07-28 | **v5 builder — Test section returns + the runs model (owner second pass, commit `3069933`).** FIVE sections: Voice · Channel · Context · **Test** · Go Live. (1) **INBOUND XOR OUTBOUND** — "one agent cannot handle both" (owner's dealership example): enforced via `enforceDirection()` on EVERY channel write path (toggle, `?dc=`, JSON apply, migration), swaps always announced with Undo. Multi-select stays for phone+web(+WhatsApp/Telegram soon). (2) **Test section** between Context and Go Live: live contextual test (inbound picks up / outbound opens the call, full persona) · **simulations auto-generated from the agent's own context** (~12 seeds specialized by prompt/vars/channels; judge = structured output `{verdict, score, reason}`; a failure names the REAL config gap — e.g. transfer off — and points only at controls that exist for the agent's channels) · **A/B split-screen** (same suite on both prompts, winner applies). Docked panel keeps Talk+Widget. (3) **Runs model** — no separate campaigns page; agents list NESTS runs per agent; **Rerun ≠ Duplicate**: Rerun = same agent, config LOCKED (truly disabled, not pointer-events), new CSV + timing only → analytics aggregate across runs (`CampaignDraft.locked`/`rerunOf`); Duplicate = editable copy. **Version history** table (property · when · old → new · who) in Go Live. (4) **Context**: knowledge/tools behind a compact "＋ Add additional context" door (auto-opens when attached); **custom-config overrides flag fields warning-yellow + DISABLED** (`AgentDraft.configOverrides`; only fields the JSON actually CHANGED lock — presence isn't override; Unlock releases). (5) **Create dialog AI-only**: static template list REMOVED; describe-box (mock AI validation, persistent aria-live) + channel chips; inference explains its picks, never force-adds a deselected phone channel. (6) "+ Add phone number" accelerator closes its loop IN the builder (`AddPhoneNumberSheet.onAdded` lists + links the number — no route-out to a fresh draft). (7) Widget config is LEAN in the builder (`WidgetStyleConfig lean` — behaviour+appearance+embed; App-Builder-style depth stays in the studio). Process: shipped through an **adversarial review workflow (4 lenses → 33 agents → 14 confirmed findings fixed pre-commit)** — spurious override locks, keyboard-editable locked reruns, Regenerate/ranIds desync, stale A/B scores, dead-end pointers, JSON sanitization, resume-cursor drift. OPEN: nomenclature ("Channel" label under exploration); number resale/KYC flows (Bandwidth) future; edit-while-run-scheduled behavior needs engineering input. |
| 2026-07-29 | **v6 corrections (owner, emphatic — commit `26a5c68`). ⚠️ LOCKS, do not re-litigate:** (1) **CHANNEL IS ONE CHOICE** — radio Inbound OR Batch calls OR Code/SDK; **top-level multi-select was NEVER asked for**. Multi-select lives ONLY INSIDE Inbound as **surfaces** (`config.inbound.surfaces`: phone + web widget together; WhatsApp/Telegram soon). **Web widget is part of Inbound** — never a peer channel. (2) **Voice = the previous controls**: LATENCY-VS-COST slider (not tier cards) + voice dropdown/browser (no quick-pick voice grid). (3) Advanced sheet stacks STT→LLM→TTS **top-to-bottom** + hosts a **Custom config (JSON)** door. (4) **KB/MCP/connectors upfront** in Context — no nesting/collapsible. (5) The Channel section shows **nothing for batch** — runs/contacts/schedule/dialing live only in Go Live. (6) **RHS docked Test panel = SIMULATIONS** (auto-generated, judge-scored), NOT the agent talk card; the live contextual talk test is inline in the Test section. (7) **A/B prompt testing: REMOVED, vetoed.** (8) Create modal asks **"What kind of a template do you want?"** (kind list under the describe box — the fallback for unsure users), not "Where should it run?". (9) Campaign editor = ONE top-to-bottom flow, contacts prominent, CSV preview collapsed. (10) **Pause in the builder header re-vetoed** — the automated fix round had re-added it (second occurrence of the 07-21 veto; graph-loop fix rounds now carry an explicit veto guardrail in FOCUS). Pre-ship 2-lens review confirmed+fixed 6 defects (surfaces-blind list rows, stale A/B palette copy, panel-close state loss, batch-vs-web migration precedence, silent deep-link swaps, dead stepManifest). |
| 2026-07-29 | **v7 minimal — "Plain Form" wins a 10-design judge round (owner: "remove whatever is unnecessary… spawn 10 vertical designs and choose the best one"; commit `2f3f6a8`).** Process: 10 parallel complete designs from distinct philosophies (zen · one-disclosure · plain-form · placeholder-carried · instrument-panel · one-question · checkout · toast-deferred · apple-settings · expert) → 5 judges (cognition load · task safety · first-run · system consistency · feel) → **Plain Form 25/50** (apple 23, placeholder 22, checkout 22; zen topped cognition but failed safety+first-run) → synthesis = plain-form spine + grafts ≥2 judges endorsed (snippet-comment honesty, one-filled-CTA-per-region, counter-form dirty state, merged scoring disclosure, warn-only capacity note, template-door dedupe, de-icon + sentence-case pass, lean props, tap-to-open guidance). **Durable rules:** (1) the builder reads as ONE flat settings form — hairline-divided rows, no cards-inside-cards, no banded accordion (removed; the rail is the only nav), no dashed drop-zones, no badge pills, no decorative icons, no mono-uppercase micro-labels; (2) helper prose survives ONLY where the field name can't carry the meaning; background knowledge lives behind the InfoHint idiom; (3) HONESTY FLOOR never trimmed away: simulated-preview disclosures, Live/pending truth (now "{n} sections edited · not live"), deploy blockers + Fix→, destructive warnings, override locks, "Draft saved", and the owner-shipped pre-flight dialog byte-identical; (4) shared components (StackTradeoffSlider, AgentIdentityCard) minimize via opt-in `lean` props — home/Playground renderings unchanged; (5) one filled CTA per region (Deploy owns Go Live's accent). Net: −449/+246 lines. |
| 2026-07-29 | **v8 — "Test Strip" wins layout judge round 2 (owner: sections collapsible again + "a way to test the agent as a step somewhere in the top, not just the simulations… re-consider 10 more options for layout"; commit `5ad9458`).** Process: 10 layout philosophies (accordion-classic · multi-open-accordion · page-tabs · stepper-top · rail-controller · master-detail · **test-strip** · split-canvas · checklist-recap · hybrid-tabs-accordion) → 4 judge lenses (Nielsen heuristics · first-run time-to-live · wayfinding · expert efficiency + lock consistency) → **Test Strip 31.6/40** (Concertina Ledger multi-open accordion 30.4; checklist-recap 27.2). Shipped = winner + judge-flag fixes + runner-up grafts. **The layout:** (1) **TEST STRIP** — slim sticky bar under the header: filled "Talk to {agent}" (expands + scrolls §4 AND starts the live test — the button does what it says, resolving the judges' teleport-disorientation flag), ghost "Run simulations" (docked panel), live verdict line "Last run: N/M passed · simulated" / "No test runs yet · simulated preview" (fed by an onRunSummary chain from the sims panel's Run all); header Test button DELETED — the strip is the one test entry region; testing is ambient ("does it work yet?" answerable at every scroll depth). (2) **Collapsible sections, multi-open** — whole plain heading row toggles (bare chevron, aria-expanded; NO banded chrome — the v7 flat language holds), **ALL OPEN by default** (the v7 one-pager stays the first-run; collapse = opt-in focus, deterministic per-agent persistence `sx:wizard_collapsed:*`, no auto-collapse heuristics), folded rows recap their values inline ("Aria · Balanced · English", "· edited" on dirty live sections — Concertina graft) so state never hides; bodies stay MOUNTED when folded (section-local state survives); rail gains Expand/Collapse all; rail clicks + ?step= deep links + pre-flight Fix links expand-then-scroll. Sticky stack rebased top-12→top-24 (topbar 48 + strip 48). NOTE: this re-adds collapse per explicit owner direction, overriding v7's "accordion deleted" — the v7 ban on banded/tinted accordion CHROME still stands; collapse is behavior, not chrome. |
| 2026-07-30 | **Section labels renamed to industry-standard developer vocabulary (owner: "I don't agree with the terms 'Channel' 'Context'… what are agent standard words at Anthropic, OpenAI or similar?"; /ux-copy round + owner mock; commit `2421080`).** Grounded against Vapi (Model·Voice·Transcriber tabs), Retell (Prompt·Knowledge Base·Deploy), ElevenLabs (Agent·Voice·Tests·Widget), OpenAI Agents SDK (instructions·tools·handoffs), Anthropic (system prompt·tools). Findings that decided it: **no platform uses "Context" as a section label** (devs read "context" as context-window; "Prompt" is the one word all five platforms share) and **none uses "Channel"** (the industry word for phone·web·SIP/code is Deploy/Deployment). **Shipped labels (owner-locked, from the owner's own mock): Voice & Models · Deployment · Prompt & knowledge · Test · Go Live**, rail grouped **CUSTOMIZE (1–3) / SHIP (4–5)** with mono-uppercase headers + per-row icons (mock overrides v7's "no group headers/icons in the rail" for the BUILDER rail). §2 "Deployment" (noun — what kind) vs §5 "Go Live" (verb — make it real) is the intended pair. Vocabulary swept everywhere a surface named a section: config drawer, pre-flight rows, ⌘K, template toast, campaign vars empty-state, publish blocks, dc-swap toasts, collapsed recaps, and ALL import-agent LANDS labels + drop-reason pointers (the DROP_REASONS honesty rule made this mandatory, not cosmetic). |
