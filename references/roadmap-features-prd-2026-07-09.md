...
| A3 | Easy ITSP Onboarding | Connect | **P0 ◆ BUILT 2026-07-09 (held for local review)** | ✗ / near-WS (1/5) | Collapses the #1 BYO-telephony pain; unblocks Connect for the BYO majority. Closes the user-test's 3×-recurring top-S2. | Built: `components/sip-quick-connect.tsx` (key → validate → enumerate → trunk → attach → **real test call**) folded into `add-phone-number-sheet` as a Quick connect \| Manual toggle; commit `eada0d8`. Design doc: `references/design/a3-itsp-connect.html`. Happy path + honesty moments shipped; 6 failure scenarios designed/judged, wiring deferred. NOT deployed. |
