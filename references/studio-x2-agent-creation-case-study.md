...
| 12 | **State-noise + batch-calls round (2026-07-09):** hide undo/reset when there's nothing to undo; drop the "General assistant" tag; badges shouldn't be pills; the "Unsaved changes" badge flickered next to the name; sticky bands floated OVER the app chrome; batch calls should show the contact list beside the settings (Figma 360-71898 half-and-half); a 500-contact upload must not make the page scroll forever. | (a) **Reset-to-live renders only when its step differs from live** — a disabled control is noise, not state. (b) Role tag removed from the lockup (rail's Voice row recaps the persona). (c) **Badge = rounded-md globally** (ui/badge.tsx), matching the chip/control radius. (d) **Edit-state has ONE fixed home: the deploy block** — the badge replaces the "not live yet" prose there (same message twice = noise); near the name it appeared/disappeared with every toggle and shoved the lockup (flicker). (e) App header z-10 → **z-30**, above the z-20 bands, so a band being pushed out slides UNDER the chrome. (f) **Batch calls = two panes** (Call setup LEFT · Contact list RIGHT, per Figma): the list is reference material you check WHILE configuring — parallel, not sequential, per the layout-by-task rule. (g) **Unbounded collections scroll INSIDE their panel** (contact preview: "first 24 of 248 rows", max-h + sticky table header) — page length must never depend on how much data the user uploads; Deploy stays one glance away. | Controls appear when actionable; every piece of state has exactly one home; chrome layers above content; lists are bounded panels, not page content. | ✅ shipped |
