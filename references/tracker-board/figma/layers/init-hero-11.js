const JOB = {"step":"init","width":1440,"height":1397,"bg":{"r":0.9921568627450981,"g":0.9882352941176471,"b":0.9882352941176471,"a":1},"sectionName":"7 · Hero screens","parentSectionId":"3122:40595","imageHash":"8b1b2ec175be2819f2f6110d9ab8532c21458865","frameName":"Hero · 11 Call sheet — SIP verdict with the fix, timed ladder (editable)","placeAt":null,"rationale":["Happy scenario = fault verdict in one look: the verdict names whose fault (Attributed to) and the fix, and the summary above agrees with it — SIP 403 here → check the trunk credentials and the IP allowlist.","'Where the time went' sums the legs (Agent → SBC · SBC → Carrier · Ring) above the ladder and marks the slowest; the deciding message carries a red rule and 'decided here' in every state.","Repeated INVITE → 503 cycles collapse to one row with ×n and 'Show all'; a call with no retained trace gets its own honest block (Call-ID for support, retention, Events pointer).","Call History gains All · With issues and attribution counts — LiveKit's 'calls with issues' done properly.","Open decisions: which fixes can deep-link (CPS limit, trunk settings)?"]}
