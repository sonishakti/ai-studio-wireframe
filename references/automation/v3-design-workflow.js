...
phase('Track')
const t = await agent(`${RULES}\n\n1. ClickUp task ${q.task_id}: add a comment "Review ready · run ${RUN} ${A.date || ''}" with prototype ${b.web}, Figma ${f.figma}, commit ${b.commit}, rationale ${JSON.stringify(d.why)}, rainy states covered ${JSON.stringify(b.shots.filter(s => s.rainy).map(s => s.caption))}, drift ${f.drift}; set status REVIEW ("Pending Review", or "in review" until renamed). Then find the card in list "0. Features · for tech" (same folder) that links to task ${q.task_id} (clickup_get_task include linked_tasks on the step card, or search the list); add a one-line comment "${q.id} delivered for review" and move it to INPROGRESS if it is TODO, or to REVIEW if every linked step card is now in REVIEW or DELIVERED.
