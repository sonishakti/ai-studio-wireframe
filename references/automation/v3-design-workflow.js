...
phase('Figma')
const f = await agent(`${RULES}\n\nLoad skill figma:figma-use before any use_figma; upload PNGs with upload_assets. In Figma file ${FIGMA}, page "${PAGE(q.p)}" (create if missing), section "${q.id} · ${q.title}" (create or replace its unlocked content) with child sections: "1 JTBD" (Sam's job from ${DIR}/01-jtbd.md: job, situation, Sam wants to, so that, happy steps, rainy list), "2 Research" (existing and new research shots from ${DIR}/shots and the row's research notes, a thin red outline on the region to look at), "3 Flow" (these screenshots in order, one frame each, caption above "Sam does …", the rationale line under it; rainy states after the happy path): ${JSON.stringify(b.shots)}, "4 Hero" (native editable frames of the 2 to 3 hero screens rebuilt from the kit components on page 31:2 with variables bound, never detached), "5 Tracker" (text with links: prototype ${b.web}, ClickUp ${q.task_url}, sheet ${SHEET}#${q.id}, commit ${b.commit}). Drift check: list every component used in section 4 and whether it has a Code Connect mapping (get_code_connect_map); compare the variables used against ${WT}/src/styles.css tokens. ${LOCKRULE} Never edit a locked section. Return the section URL and the drift result.`,
