...
|---|---|---|
| ClickUp folder **Design Tracker** (Product space, id `90119281625`) | Three lists, same four columns (To-do · In progress · Pending Review · Delivered): **0. Features · for tech** = 14 feature cards F1 … F14 (what each builds, API surface, blocked by, due = design ready); **1. V3** `901115453665` = the 42 job steps P0.1 … P3.6 in the order they get done; **2. Future Sprints** `901115453666` = the 28 roadmap features 01 … 28, after v3 | Tech reads 0. Features; owner, product and FE read 1. V3; the agent reads 1. V3 |
| ClickUp doc **Studio v3 PRD** `853xy-81151` | The PRD for the whole team: overview with the features table, one page per phase, Future Sprints, Funnel and KPIs, Decisions and gaps, Vocabulary, How the design loop runs; regenerated by `prd-v3-build.py` (clickup/doc/*.md) | Everyone |
